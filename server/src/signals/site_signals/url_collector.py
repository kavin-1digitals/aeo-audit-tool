...
MAX_CONCURRENT_REQUESTS = 20  # 🔥 control parallelism
...
async def crawl_site(url: str) -> CrawlResult:
    ...
    async with httpx.AsyncClient(
        ...
        js_enabled = is_javascript_heavy(soup)

        logger.info(f"JS Enabled: {js_enabled}")
        ...
        if not js_enabled:
            ...
            # depth 2 (parallel per parent batch)
            random.shuffle(depth1_pages)
            selected = depth1_pages[:MAX_DEPTH2_PAGES]

            for parent in selected:
                soup = BeautifulSoup(parent.content, "html.parser")
                links = list(filter_internal_links(extract_links(soup, parent.url), url))
                random.shuffle(links)

                tasks = [
                    fetch_httpx(client, link, sem)
                    for link in links[:MAX_DEPTH3_PER_PAGE]
                    if normalize_url(link) not in visited
                ]
        ...
        # ---------------------------
            ...
            for link in links[:JS_FALLBACK_PAGES]:
                norm = normalize_url(link)
                if norm in visited:
                    continue
